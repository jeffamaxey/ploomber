...
def test_serialize_fallback_invalid():
    ...
    expected = ("Invalid fallback argument 'invalid' in "
...
def test_serialize_missing_provider(monkeypatch, provider):
    ...
    expected = (f"Error serializing with function 'serializer_fallback'."
...
def test_signature_serializer():
    ...
    expected = ("Expected serializer 'serializer_wrong_signature' to "
...
def test_serializer_error_on_invalid_types(tmp_directory, extension):
    ...
    assert (f"Error serializing product File('a{extension}') to {extension}"
...
def test_unserialize_missing_provider(monkeypatch, provider):
    ...
    expected = (f"Error unserializing with function 'unserializer_fallback'."
                f" {provider} is not installed")
    assert str(excinfo.value) == expected
...
def test_signature_unserializer():
    ...
    expected = ("Expected unserializer 'unserializer_wrong_signature' to "
...
@pytest.mark.parametrize('extension, serializing_fn', [
    ['.csv', 'to_csv'],
    ['.parquet', 'to_parquet'],
])
...
def test_unserialize_error_if_pandas_missing(tmp_directory, extension,
                                             serializing_fn, monkeypatch):
    ...
    expected = (f'Error using {extension} default unserializer: '
...
@pytest.mark.parametrize('decorator_factory, name', [
...
def test_un_serializer_error_on_default_invalid_value(decorator_factory, name):
    ...
    assert (str({'.invalid', 'another'}) in str(excinfo.value)
            or str({'another', '.invalid'}) in str(excinfo.value))
...
def test_un_serializer_error_on_extension_mapping_and_defaults_overlap(
    ...
    assert f'Error when adding @{name}' in str(excinfo.value)
    assert 'my_fn' in str(excinfo.value)
    assert (str({'.txt', '.csv'}) in str(excinfo.value)
            or str({'.csv', '.txt'}) in str(excinfo.value))
